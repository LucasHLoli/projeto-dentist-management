...
const TABS = ['Atendimento', 'Financeiro/Clínico'] as const;
...
export default function NovoAtendimentoModal({ onClose, onSave, nextId }: Props) {
  ...
  const [plano, setPlano] = useState<Plano>('Particular');
  ...
  const [pediuRecibo, setPediuRecibo] = useState(false);
  const [limpeza, setLimpeza] = useState(false);
  const [descricaoQueixa, setDescricaoQueixa] = useState('');
  ...
  const totalProcedimentos = selectedProcedures.reduce((sum, p) => sum + p.valor2026, 0);
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                            ...
                            {(['1F', '2F', '3F', '4F'] as const).map(f => (
                              <button
                                ...
                                style={{
                                  ...
                                  background: restauracaoState.faces === f ? 'var(--accent-teal)' : 'transparent',
                                  color: restauracaoState.faces === f ? 'white' : 'var(--text-secondary)',
                                  cursor: 'pointer',
                                  fontSize: '0.82rem',
                                  fontWeight: 500,
                                }}
                        ...
                        <div style={{ display: 'flex', gap: '6px', alignSelf: 'flex-end' }}>
                          ...
                          <button
                            ...
                            onClick={() => { setRestauracaoState(null); setRestauracaoProcId(null); }}
                          >
                            Cancelar
                          </button>
                        </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '10px' }}>
                      {selectedProcedures.map(p => (
                        <span key={p.id} style={chipStyle}>
                          {p.nome}
                          <span style={{ marginLeft: '4px', opacity: 0.7, fontSize: '0.75rem' }}>({fmt(p.valor2026)})</span>
                          <button
                            type="button"
                            onClick={() => removeProcedure(p.id)}
                            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0 0 0 4px', color: 'inherit', display: 'flex', alignItems: 'center' }}
                          >
                            <X size={12} />
                          </button>
                        </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
              <input
                ...
                id="recibo"
                checked={pediuRecibo}
                onChange={e => setPediuRecibo(e.target.checked)}
                style={{ width: '16px', height: '16px', cursor: 'pointer' }}
              />
            ...
            {plano === 'Particular' && (
              <div>
                ...
                {selectedProcedures.length > 0 && (
                  <div style={{ marginBottom: '10px', padding: '8px 12px', background: 'var(--bg-glass)', borderRadius: '8px', fontSize: '0.85rem', display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Total procedimentos:</span>
                    <strong style={{ color: 'var(--accent-teal)' }}>{fmt(totalProcedimentos)}</strong>
                  </div>
                )}
